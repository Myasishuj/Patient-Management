...
-- -- Adds the `birth_date` column to the `patient` table.
-- -- We use a `DEFAULT` value to ensure existing rows don't cause a `NULL not allowed` error.
-- ALTER TABLE patient ADD COLUMN birth_date date NOT NULL DEFAULT '1900-01-01';
--
-- -- Adds the `register_date` column to the `patient` table.
-- -- We use a `DEFAULT` value to ensure existing rows don't cause a `NULL not allowed` error.
-- ALTER TABLE patient ADD COLUMN register_date date NOT NULL DEFAULT '1900-01-01';

-- Insert well-known UUIDs for specific patients
INSERT INTO patient (id, name, email, address, birth_date, register_date)
